...
def standardized_betas(X, y):
    ...
    beta_standardized = np.linalg.inv(X_standardized.T.dot(
        X_standardized)).dot(X_standardized.T).dot(y_standardized)
...
def my_pred(X, Y, beta_standardized):
    beta0 = np.mean(
        Y) - np.sum(beta_standardized[i]*np.mean(X.iloc[:, i]) for i in range(X.shape[1]))
    _pred = beta0
...
def my_plot(X, Y, reg_pred, r2, beta_standardized):
    ...
    ax.plot([reg_pred.min(), reg_pred.max()], [reg_pred.min(), reg_pred.max()],
            linestyle='-', lw=2, label='Regresión Lineal', color='red')
